...
class TdSessionCallBackI : public   TdSessionCallBack
{
	public:
		...
		virtual ::Ice::Int onTradingday(const ::std::string& fundid, const string& tradingday,  const ::Ice::Current&  cur)
		{
			LOG_INFO("fundid=" << fundid << " tradingday=" << tradingday);
		}
};
...
int Client::run(int, char*[])
{
	try
	{
		...
		TdSessionPrx server = TdSessionPrx::checkedCast(communicator()->propertyToProxy("TDSUNGARD.Proxy"));
...
*/		
		...
		LOG_DEBUG("fundid=1248 tradingday=" << server->getTradingday());
		/*
...
*/
		...
		//cout << "cancleOrder=" << server->cancleOrder("1248", "IF1612", "1104836029", "10003582") << endl;
		...
		for(; itro!=o.end(); itro++)
		{
			...
			//cout << "cancleOrder=" << server->cancleOrder(fundid, ins, privateno, orderid) << endl;

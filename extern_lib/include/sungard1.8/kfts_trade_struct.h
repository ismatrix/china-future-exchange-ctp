...
#define EMPTY_NO        -1
	...
	* 撤单方式二:MarketCode+SessionID+PrivateNo 根据私有号撤单；<br>
	...
	*/
	...
	typedef struct _STOrderInfo
	{
		...
		TMarketOrderIDType	MarketOrderID;				    ///< 交易所系统委托流水号
	...
	typedef struct _STAccountInfo
	{
		...
		TAmountType		PositionProfit;				///< 盯市盈亏
		TAmountType		Balance;				    ///< 动态权益
	...
	typedef struct _STPositionInfo
	{
		...
		TLargeVolumeType split_merge_available; /// <分拆合并可用数量
		TAmountType      OpenAveragePrice;  /// <开仓均价
